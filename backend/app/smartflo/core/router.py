...
class EventRouter:
    ...
    async def dispatch(
        ...
        **kwargs
        ...
        Args:
            event: Validated Pydantic event object
        ...
        logger.debug(f"Dispatching {event_type} event to handler: {handler.__name__}")
        
        try:
            result = await handler(event, websocket, **kwargs)
            return result
        except Exception as e:
            logger.error(f"Error in handler for {event_type}: {str(e)}", exc_info=True)
            raise
