...
def get_question_text(session):
    ...
    return text.replace("{{customer_name}}", session.get("customer_name", ""))
...
def handle_edit_response(session, user_input):
    ...
    if edit_info:
        ...
        logger.info(f"  - Old Value: {session.get(field, 'N/A')}")
        ...
        if field in session:
            ...
            session["summary_confirmed"] = True
            logger.info(
                f"✅ Successfully updated field '{field}' - moving to closing phase"
            )
            logger.info("=" * 80)
            return "CLOSING"
        else:
