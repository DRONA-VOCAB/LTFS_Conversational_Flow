        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    return {"status": "unknown", "message": "TTS service health endpoint not available"}
            else:
                return {"status": "unhealthy", "message": f"TTS returned {response.status_code}"}
    except httpx.TimeoutException:
        return {"status": "unknown", "message": "TTS service timeout - may be unavailable"}
    except Exception as e:
        logger.warning(f"TTS health check failed: {e}")
        return {"status": "unknown", "message": f"TTS check failed: {str(e)}"}
